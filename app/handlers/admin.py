...
async def main(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(loc.get_text("Выберите действие"), reply_markup=kbc.admin_keyboard())
...
async def handle_add_account_btn(
    ...
    await callback.message.answer(loc.get_text("Введите номер телефона аккаунта:"))
